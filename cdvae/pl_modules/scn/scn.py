...
from cdvae.pl_modules.scn.sampling import CalcSpherePoints
from cdvae.pl_modules.scn.smearing import (
    GaussianSmearing,
    LinearSigmoidSmearing,
    SigmoidSmearing,
    SiLUSmearing,
)
from cdvae.pl_modules.scn.spherical_harmonics import SphericalHarmonicsHelper
...
class SphericalChannelNetwork(nn.Module):
    ...
    def __init__(
        ...
        self.energy_fc3 = nn.Linear(self.sphere_channels_reduce, self.sphere_channels_reduce)
    ...
    def forward(self, z, frac_coords, atom_types, num_atoms, lengths, angles,
        ...
        self.device = frac_coords.device
        self.num_atoms = frac_coords.shape[0]
        self.batch_size = len(num_atoms)
        # torch.autograd.set_detect_anomaly(True)

        outputs = self._forward_helper(
            z, frac_coords, atom_types, num_atoms, lengths, angles,
            edge_index, to_jimages, num_bonds, conditional
        )
    ...
    def _forward_helper(
            self, z, frac_coords, atom_types, num_atoms, lengths, angles,
            edge_index, to_jimages, num_bonds, conditional=None):
        # TODO: Conditional
        ...
        (
            ...
            _,  # neighbors
        ) = self.generate_graph(pos, lengths, angles, num_atoms, edge_index, to_jimages,
            num_bonds)
        ...
        edge_rot_mat = self._init_edge_rot_mat(
            pos, edge_index, edge_distance_vec
        )
        ...
        node_energy = node_energy.view(-1, self.num_sphere_samples, self.sphere_channels_reduce)
        node_energy = torch.sum(node_energy, dim=1) / self.num_sphere_samples
        batch_size = torch.max(batch) + 1
        energy = scatter(
            node_energy, batch, dim=0, dim_size=batch_size, reduce="mean"
        ...
        if torch.min(edge_vec_0_distance) < 0.0001:
            ...
            print(
                "Error edge_vec_0_distance: {} {} {} {} {}".format(
                    ...
                    edge_index[0, minidx],
                    edge_index[1, minidx],
                    pos[edge_index[0, minidx]],
                    pos[edge_index[1, minidx]],
                )
            )
    ...
    def generate_graph(
        ...
        cell_offset_distances = None  # TODO
        neighbors = None  # TODO
        return (
            edge_index,
            edge_dist,
