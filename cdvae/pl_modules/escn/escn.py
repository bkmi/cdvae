...
from cdvae.pl_modules.escn.so3 import SO3_Embedding
from cdvae.pl_modules.escn.so3 import SO3_Rotation
...
class eSCN(nn.Module):
    ...
    def __init__(
        ...
        self.fc_z = nn.Linear(hidden_channels, self.sphere_channels)
    ...
    def forward(self, z, frac_coords, atom_types, num_atoms, lengths, angles,
        ...
        atomic_numbers = atom_types.long()
        ...
        (
            ...
            neighbors,
        ) = self.generate_graph(pos, lengths, angles, num_atoms, edge_index, to_jimages, num_bonds)
        ...
        node_energy = node_energy.reshape(pos.shape[0], -1)
        energy = node_energy  # * 0.001

        # energy = torch.zeros(len(num_atoms), device=pos.device)
        # print(batch)
...
class EnergyBlock(torch.nn.Module):
    ...
    def __init__(
        ...
        self.fc3 = nn.Linear(self.num_channels, self.num_channels, bias=False)
